"""
Quantum-Inspired Kernel Matrix Computation and Evaluation for Synthetic Datasets

Author: Dr. techn Sebastian Raubitzek MSc. BSc.

This Python script performs a comprehensive analysis of quantum-inspired kernel matrices using synthetic datasets, focusing on various symmetry group feature maps. The goal is to compute and evaluate kernel matrices based on quantum kernel estimators (QKE) and compare them with classical machine learning models, such as CatBoost, for binary classification tasks.

### Key Components:
1. **Symmetry Group-Based Feature Maps**:
    - The script applies feature maps derived from various symmetry groups (SO, SL, SU, GL, U, O, etc.) to the input dataset. Each group defines a unique transformation of the feature space, allowing for different ways to encode classical data into quantum-inspired kernel matrices.
    - These feature maps are central to the method of quantum kernel estimation described in "Quantum-inspired Kernel Matrices: Exploring Symmetry in Machine Learning" (Raubitzek et al., 2024).

2. **Kernel Matrix Computation**:
    - **Training Kernel Matrix**: Computed using the feature map applied to the training data. The kernel matrix captures the similarity between transformed feature vectors, potentially in a high-dimensional space.
    - **Test Kernel Matrix**: Computed by applying the feature map to both training and test data, enabling out-of-sample predictions using a precomputed kernel matrix.
    - The kernel matrices can be real or complex depending on the nature of the transformation, and they are checked and converted to real matrices when needed.

3. **Synthetic Dataset Loading**:
    - The script loads synthetic datasets generated separately, which consist of three features (`x0`, `x1`, `x2`) and binary class labels. These datasets simulate classification tasks based on symmetry group transformations, with feature separation reflecting distinct parts of the Lie manifold.
    - The dataset is shuffled and split into training and test sets for model evaluation.

4. **Machine Learning Models**:
    - **Support Vector Machine (SVM)**: A precomputed kernel SVM is used for classification tasks, utilizing the quantum-inspired kernel matrix generated by the feature maps.
    - **CatBoost Classifier**: A state-of-the-art classical machine learning model is used as a baseline for comparison. CatBoost is trained and evaluated on the same data to assess the effectiveness of quantum-inspired approaches.

5. **Evaluation Metrics**:
    - The script computes standard classification metrics to assess model performance:
        - **Accuracy**: Correct predictions divided by the total number of samples.
        - **Precision**: Ratio of true positives to predicted positives.
        - **Recall**: Ratio of true positives to actual positives.
        - **F1-Score**: Harmonic mean of precision and recall.
    - Both SVM and CatBoost models are evaluated and compared based on these metrics for each dataset.

6. **Kernel Matrix Visualization**:
    - The script generates and saves heatmaps of the computed kernel matrices to visualize the data's structure under different symmetry group feature maps. These visualizations provide insights into how well different symmetries separate the data.
    - Additionally, vector plots in 2D and 3D are created to display the transformed feature vectors.

7. **Results Logging**:
    - The script logs detailed performance metrics, kernel matrix computation times, and training/prediction times for each symmetry group. These results are saved in both CSV and JSON formats for further analysis.
    - The results include:
        - Number of features
        - Group type and its generators
        - Kernel matrix computation time
        - SVM and CatBoost metrics (accuracy, precision, recall, F1-score)

### Example Workflow:
1. Load a synthetic dataset and apply various symmetry group-based feature maps to compute the kernel matrices.
2. Train an SVM classifier using the precomputed kernel matrix and evaluate its performance on test data.
3. Train a CatBoost classifier on the same dataset for comparison.
4. Log and visualize the results, saving kernel matrices and performance metrics for analysis.

### Context and Purpose:
- This script builds upon the theoretical framework of quantum kernel estimators, expanding their use to include symmetry group-based feature maps. By leveraging the structure of Lie groups, it aims to demonstrate the potential quantum advantage in encoding and classifying complex datasets.
- The synthetic datasets are specifically designed to highlight the impact of these transformations, allowing researchers to evaluate different symmetry-based approaches in a controlled environment.
- Results are visualized and saved for further exploration and publication, providing a comprehensive analysis of quantum-inspired feature maps in machine learning.

### Notes:
- The scaling parameter controls the normalization of input data before feature transformation, affecting the range of values that the feature maps operate on (e.g., [0,1], [0,π], [0,2π]).
- Adjustments can be made to the number of samples, scaling, and other hyperparameters to experiment with different settings.

References:
- Raubitzek et al., 2024, Physics Letters A 525, "Quantum-inspired Kernel Matrices: Exploring Symmetry in Machine Learning"
- CatBoost documentation for classical machine learning model comparisons.
"""
