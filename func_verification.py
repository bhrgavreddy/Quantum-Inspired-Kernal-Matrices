"""
Lie Group Generator Verification for Quantum-Inspired Symmetry Transformations

Author: Dr. techn Sebastian Raubitzek MSc. BSc.

This module provides verification functions for generators of various Lie groups (SU(n), SO(n), SL(n), GL(n), U(n), O(n), T(n)). These groups are fundamental in physics and machine learning, where they describe symmetry operations and transformations.

### Key Verifications:
1. **Skew-Hermitian/Skew-Symmetric**:
   - Ensures generators are skew-Hermitian (for SU, U) or skew-symmetric (for SO, O), preserving symmetries.

2. **Tracelessness**:
   - Verifies that each generator's trace is zero (for SU, SL).

3. **Algebra Closure**:
   - Confirms that the Lie algebra is closed under the commutator operation.

4. **Unitarity/Orthogonality**:
   - Ensures exponentiated generators yield unitary (for SU, U) or orthogonal (for SO, O) matrices.

5. **Correct Generator Count**:
   - Verifies the number of generators matches the expected value for each group.

6. **Determinant**:
   - For groups like SU(n) and SL(n), exponentiated matrices must have a determinant of 1.

### Functions:
- **`verify_SU_generators(n)`**: Checks the generators of SU(n) for skew-Hermitian property, tracelessness, algebra closure, unitarity, and determinant 1.
- **`verify_SO_generators(n)`**: Verifies skew-symmetry, algebra closure, orthogonality, and determinant +1 for SO(n).
- **`verify_T_generators(n)`**: Ensures translation group generators have correct structure and algebra closure.
- **`verify_GL_generators(n)`**: Confirms algebra closure, invertibility, and dimensionality for GL(n).
- **`verify_SL_generators(n)`**: Verifies algebra closure, determinant 1, and correct generator count for SL(n).
- **`verify_U_generators(n)`**: Ensures unitarity, algebra closure, and correct generator count for U(n).
- **`verify_O_generators(n)`**: Checks skew-symmetry, orthogonality, algebra closure, and dimensionality for O(n).

References:
- Raubitzek et al., 2024, "Quantum-inspired Kernel Matrices: Exploring Symmetry in Machine Learning"
- Hall, Brian C., "Lie Groups, Lie Algebras, and Representations"
"""
