"""
Quantum-Inspired Kernel Matrix Computation with Symmetry Feature Maps

Author: Dr. techn Sebastian Raubitzek MSc. BSc.

This Python script implements a quantum-inspired approach to computing kernel matrices for machine learning classification tasks, with an emphasis on symmetry feature maps derived from Lie groups. The methodology draws inspiration from quantum machine learning, utilizing the concept of Quantum Kernel Estimators (QKE) and extending it beyond the typical SU(2) symmetry to a broader spectrum of symmetry groups.

### Main Components:
1. **Feature Maps**: Symmetry-based feature maps that encode classical data into higher-dimensional feature spaces using Lie groups such as SO(n), SL(n), SU(n), GL(n), U(n), and O(n). These feature maps are used to transform input data into vectors that capture essential symmetries.

2. **Kernel Matrix Computation**:
    - **Train Kernel Matrix**: This is computed based on the feature map applied to the training data. The kernel values are calculated using the dot product between transformed feature vectors, capturing both real and complex components.
    - **Test Kernel Matrix**: Similar to the training kernel matrix, this is computed by applying the feature map to both training and test datasets, allowing for out-of-sample predictions.

3. **Machine Learning Models**:
    - **Support Vector Classifier (SVC)**: Utilizes precomputed kernel matrices to perform classification tasks. The SVM model leverages these custom kernel matrices, which are based on symmetry transformations, for better data representation.
    - **CatBoost Classifier**: A standard classical machine learning classifier used for comparison with the quantum-inspired approach.

4. **Symmetry Groups**: This script explores various Lie group families (SO, SL, SU, GL, U, O) for data encoding, allowing flexibility in feature transformation. Each symmetry group is defined by its generators, which are applied to the input data to create transformed feature vectors that reflect the inherent symmetries of the data.

5. **Dataset Handling**:
    - The script supports multiple datasets, with built-in handling for non-numeric data through label encoding and normalization. The datasets are fetched from OpenML and processed based on the selected feature map.

### Parameters:
- `output_real`: A boolean flag indicating whether the kernel matrix output should be real.
- `scaling`: Defines the scaling range for data normalization. Different scaling options correspond to different intervals, such as [0, 1], [0, pi], and [0, 2pi].

### Metrics:
- The script evaluates model performance using standard classification metrics: accuracy, precision, recall, and F1-score. These metrics are computed for both the SVM and CatBoost models.

### Kernel Matrix Visualization:
- The script saves and visualizes the computed kernel matrices using heatmaps, providing insights into the structure of the transformed data under different symmetry groups. This visualization aids in understanding how different symmetries impact the kernel matrix and model performance.

### Example Workflow:
1. Load a dataset and apply the specified feature map.
2. Compute the kernel matrix for both training and testing data.
3. Train a support vector machine using the kernel matrix.
4. Evaluate the performance of the model and compare it with a classical CatBoost classifier.
5. Visualize the kernel matrix to understand the impact of different symmetry transformations on the data.

### Notes:
- This script builds on the theoretical foundation of Quantum Kernel Estimators (QKE) and extends the use of Lie groups for feature mapping, as detailed in the accompanying research paper "Quantum-inspired Kernel Matrices: Exploring Symmetry in Machine Learning" (Raubitzek et al., 2024). The code leverages these concepts to provide a flexible framework for experimenting with symmetry-based data encoding and classification.
